...
export const Home: React.FC<HomeProps> = ({ activities, user, isEnglish, onJoinActivity, setView }) => {
  ...
  const categories = [
    { label: isEnglish ? 'Academic' : '学术讲座', icon: BookOpen, color: 'bg-blue-50 text-blue-600' },
    { label: isEnglish ? 'Social' : '社团活动', icon: Users, color: 'bg-green-50 text-green-600' },
    { label: isEnglish ? 'Culture' : '文娱艺术', icon: Palette, color: 'bg-purple-50 text-purple-600' },
    { label: isEnglish ? 'Sports' : '体育竞技', icon: Trophy, color: 'bg-orange-50 text-orange-600' },
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat, idx) => (
              <button key={idx} className="flex flex-col items-center justify-center p-4 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all bg-white group">
